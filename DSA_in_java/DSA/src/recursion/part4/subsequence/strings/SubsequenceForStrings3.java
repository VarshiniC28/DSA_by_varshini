package recursion.part4.subsequence.strings;

import java.util.ArrayList;

public class SubsequenceForStrings3 {
	
	// for string str = "ab" print all subsequences that also contain ASCII value of the char
	static void subseq(String p, String up) {
		if(up.isEmpty()) {
			System.out.println(p);
			return;
		}
		
		char ch = up.charAt(0);
		
		subseq(p + ch , up.substring(1)); //ignoring a char
		subseq(p, up.substring(1)); //taking a char
		subseq(p + (ch + 0), up.substring(1));
		
	}
	
	//---------------------------------------------------------------------
	
	static ArrayList<String> subseq2(String p, String up){
		if(up.isEmpty()) {
			ArrayList<String> list = new ArrayList<>();
			list.add(p);
			return list;
		}
		
		char ch = up.charAt(0);
		
		ArrayList<String> first = subseq2(p + ch, up.substring(1));
		ArrayList<String> sec = subseq2(p, up.substring(1));
		ArrayList<String> third = subseq2(p + (ch + 0), up.substring(1));
		
		first.addAll(sec);
		first.addAll(third);
		
		return first;
	}

	//------------------------------------------------------------------
	
	
	public static void main(String[] args) {

		subseq("", "ab"); 
		
		System.out.println();
		
		System.out.println(subseq2("", "ab"));
	}

}
